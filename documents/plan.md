...
> **実装後の乖離（2026-07-19 追記）**: 本ファイルは実装着手前の初版設計計画書であり、歴史的記録として当時の内容を保持している。実装完了後の現状は以下の点で本文の記述と異なる。
>
> - シンタックスハイライトは `highlight.js` ではなく **Shiki** を採用（§2.2, §3.2, §4.3）
> - Deployment Target は macOS 13.0 ではなく **macOS 14.0**（プロジェクト設定で確認）
> - テーマは Solarized / Dracula / Nord を含む6種として設計されたが、実装済み（CSSファイルが存在する）のは **GitHub Light / GitHub Dark の2種のみ**。`MarkdownTheme.swift` には残り4種の定義が残っているが、対応するCSSファイルが `Resources/Web/themes/` に存在しないため選択すると壊れる（未修正の既知バグ）
> - v1.1.0で新規ファイル作成機能（⌘N）、v1.2.1でMarkdown/Mermaid記法ヘルプウィンドウを追加。本計画書には未記載
> - §9で予告されている `documents/README.md`・`documents/architecture.md` は作成されていない（README.md/README.ja.mdはリポジトリルートに存在）
> - 複数タブ・最近使ったファイルメニュー・絵文字記法（§3.1, §3.8）など、本文に記載のある機能の実装状況は未確認

---

## 1. 設計概要

macOS ネイティブの Markdown ビューアアプリ。左パネルで目次（TOC）を表示し、右メインエリアで Markdown をレンダリングする。GFM 準拠のレンダリングに加え、シンタックスハイライト・数式（KaTeX）・Mermaid 図・テーブルをサポートする。
...
| 機能 | 実装 |
|------|------|
| 目次（TOC） | swift-markdown でヘッダノードを抽出し、`OutlineView`（`List` の再帰）で H1〜H4 を階層表示。クリックで WKWebView のアンカーへスクロール（`WKWebView.evaluateJavaScript` で `scrollIntoView` 実行） |
...
**目標**: 目次サイドバー、テーマ切り替え、ページ内検索を実装する。
...
| 2.4 | `SidebarView` 実装 | `Views/Sidebar/SidebarView.swift` | TOC を表示する `TableOfContentsView` をラップ |
| 2.5 | テーマ CSS 作成 | `Resources/Web/themes/*.css` | github-light / github-dark / solarized-light / solarized-dark / dracula / nord の6ファイルを作成 |
| 2.6 | `RenderViewModel` テーマ対応 | `ViewModels/RenderViewModel.swift` | `@Published var theme: MarkdownTheme`。変更時に `MDViewer.setTheme(name:)` を JS に送信 |
| 2.7 | `SearchBarView` 実装 | `Views/Toolbar/SearchBarView.swift` | Cmd+F でフォーカス。`WKFindInteraction` を `WebRendererView` に紐付け。件数をステータスバーに表示 |
| 2.8 | `AppStorage` 永続化 | `ViewModels/` 各ファイル | theme, fontSize, sidebarWidth, lastOpenedURL を AppStorage に保存 |
| 2.9 | `FileWatcher` 実装 | `Utilities/FileWatcher.swift` | `DispatchSourceFileSystemObject` で監視。変更検出後 0.5s デバウンスで `DocumentViewModel.reload()` を呼び出す |

**Phase 2 完了基準**: TOC クリックでスクロール、テーマ切り替え、Cmd+F 検索が動作する。
